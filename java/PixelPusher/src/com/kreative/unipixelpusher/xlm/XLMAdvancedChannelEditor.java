...
public class XLMAdvancedChannelEditor extends JPanel {
	private static final long serialVersionUID = 1L;
	
	private final XLMChannel channel;
	...
	private JPanel makeMainPanel() {
		...
		final SpinnerNumberModel msSpinnerModel = new SpinnerNumberModel(channel.getMsPerFrame(), 20, 60000, 20);
		...
		msSpinnerModel.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				channel.setMsPerFrame(msSpinnerModel.getNumber().intValue());
			}
		});
	...
	private class XLMAdvancedPatternEditor extends JPanel {
		...
		public XLMAdvancedPatternEditor() {
			...
			for (int i = 0; i < channel.getPatternLength(); i++) {
			...
			add.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					int l = channel.getPatternLength() + 1;
					channel.setPatternLength(l);
					colorList.removeAll();
					for (int i = 0; i < l; i++) {
	...
	private class XLMAdvancedPatternColorEditor extends JPanel {
		...
		public XLMAdvancedPatternColorEditor(int index) {
			...
			this.colorPopup.setSelectedItem(channel.getPatternColor(index));
			this.countSpinnerModel = new SpinnerNumberModel(channel.getPatternCount(index), 1, 250, 1);
