...
public class XLMVisualizer extends PixelSequenceVisualizer {
	...
	public XLMVisualizer() {
		this(new XLMSequence());
	}
	
	public XLMVisualizer(PixelSequence sequence) {
		super(sequence);
	...
	public synchronized void paintPixelSet(Graphics g, int x, int y, int w, int h, PixelSequence sequence) {
		...
		double ab = 90.0 - 180.0 / barColors.length;
		double xc = x + w / 2.0;
		double yc = y + h / 2.0;
		double ir = 100.0;
